...
for csv_file in sorted(os.listdir("DEMO\collected_data")):
    ...
    df_collected["end"] = df_collected["end"].dt.strftime("%Y-%m-%dT%H:%M:%S.000+00:00")
...
for i,row in list(all_df_collected.iterrows())[::-1]:
    ...
    if current_trace != row["trace"] and current_trace and current_trace != "s1s1":
        log_template = log_template[:142] + add_trace + "\n" + log_template[142:]

    if current_trace != row["trace"]:
        add_trace = """    <trace>
        <string key="concept:name" value=""/>
    </trace>"""
        add_trace = add_trace[:54] + row["trace"] + add_trace[54:]
        current_trace =  row["trace"]
    ...
    add_event += get_activity[0][2]  + event_template[151:155]
    if len(get_activity) == 2:
        add_event += f'''            <string key="{get_activity[1][0]}:{get_activity[1][1]}" value="{row['location']}"/>\n'''
    add_event += event_template[155:]
    add_trace = add_trace[:62] + add_event + add_trace[62:]

    if current_trace == "s1s1":
        if i == 0:
            log_template = log_template[:142] + add_trace + "\n" + log_template[142:]
